...
int main(void){
  ...
  for(int i = 0; i<(N+1); i++){
    if(i<(N/2+1)){
      w_1[i]  = 1.0/0.1;
      w_2[i]  = 0.0;
      w_3[i]  = energy(1.0/(1.4*0.1),1.0/0.1,0.0);
    }
    else{
      w_1[i] = 1.0;
      w_2[i]  = 0.0;
      w_3[i]  = energy(1.0/1.4,1.0,0.0);
    }
  }
...
float energy(float pressure, float density, float velocity){
  return ((5.0/2)*pressure + (density/2.0)*velocity*velocity);
}
...
float F2(float w1, float w2, float w3){
  return ((w2*w2)/w1) + (2.0/5)*(w3 - ((w2*w2)/(2.0*w1)));
}    
float F3(float w1, float w2, float w3){
  return (w3 + (2.0/5)*(w3 - ((w2*w2)/(2.0*w1))))*(w2/w1);
}
